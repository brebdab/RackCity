...
class RackRangeSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if ("num_end" in data) and (data["num_end"] < data["num_start"]):
            message = (
                "invalid rack number range from "
                + str(data["num_start"])
                + " to "
                + str(data["num_end"])
            )
        ...
        if ("letter_end" in data) and (data["letter_end"] < data["letter_start"]):
            message = (
                "invalid rack row range from "
                + data["letter_start"]
                + " to "
                + data["letter_end"]
            )
    ...
    def get_num_racks_in_range(self):
        ...
        if "num_end" in self.validated_data:
            num_col = (
                1 + self.validated_data["num_end"] - self.validated_data["num_start"]
            )
        else:
            num_col = 1
    ...
    def get_number_range(self):
        ...
        """
        if "num_end" in self.validated_data:
            return (self.validated_data["num_start"], self.validated_data["num_end"])
        else:
            return (self.validated_data["num_start"], self.validated_data["num_start"])
