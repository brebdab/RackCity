...
class AuthFailure(Enum):
    ...
    DATACENTER = (
        "You do not have global asset permission required to "
        + "create, modify, or delete datacenters."
    )
    RACK = (
        "You do not have the asset permission required to create or "
...
class BulkFailure(Enum):
    IMPORT = (
        "File import failed. " + "See in-app documentation for format specifications."
    )
    IMPORT_COLUMNS = (
...
def get_rack_exist_failure(racks):
    if len(racks) < 5:
        failure_message = (
            " because the following racks already exist: " + get_rack_list(racks)
        )
    else:
        failure_message = (
...
def get_rack_do_not_exist_failure(rack_names):
    if len(rack_names) < 5:
        failure_message = " because the following racks do not exist: " + ", ".join(
            rack_names
        )
    else:
        failure_message = (
...
def get_rack_with_asset_failure(racks):
    if len(racks) < 5:
        failure_message = " because the following racks contain assets: "
        failure_message += get_rack_list(racks)
    else:
        failure_message = " because at least the following racks contain assets: "
        failure_message += get_rack_list(racks[0:5]) + ", ..."
    return failure_message


def get_rack_list(racks):
    return ", ".join([str(rack.row_letter) + str(rack.rack_num) for rack in racks])
...
def get_invalid_paginated_request_response(query_params):
    ...
    if not query_params.get("page"):
        ...
        errors.append("Must specify field 'page_size' on " + "paginated requests.")
    else:
        try:
            page_size = int(query_params.get("page_size"))
        except ValueError:
            errors.append(
                "Must specify integer 'page_size' on " + "paginated requests."
            )
        if page_size <= 0:
            errors.append("Field 'page_size' must be an integer " + "greater than 0.")
    if len(errors) > 0:
        return JsonResponse(
            {
