...
def log_action(user, related_element, action, change_plan=None):
    ...
    if isinstance(related_element, Asset):
        ...
        element_name = " ".join([related_element.vendor, related_element.model_number])
    ...
    if change_plan:
        log_content += " in the execution of Change Plan '" + change_plan.name + "'"
    log = Log(
        date=date,
...
def log_delete(user, element_type, element_name):
    ...
    log = Log(date=date, log_content=log_content, user=user,)
    log.save()


def log_execute_change_plan(
    user, change_plan_name, num_created, num_modified, num_decommissioned
):
    date = datetime.now()
    log_content = " ".join(
        [
            ...
            ElementType.USER.value,
            user.username,
            "executed Change Plan" "'" + change_plan_name + "':",
            str(num_created),
            "assets created,",
            str(num_modified),
            "assets modified,",
