...
def netid_login(request):
    ...
    if "access_token" not in request.data:
        return JsonResponse(
            {
                "failure_message": Status.ERROR.value + UserFailure.NETID_LOGIN.value,
    ...
    return JsonResponse({"key": token.key}, status=HTTPStatus.OK,)
...
def user_many(request):
    ...
    return get_many_response(
        User, UserSerializer, "users", request, default_order="username",
    )
...
def i_am_admin(request):
    ...
    """
    return JsonResponse({"is_admin": request.user.is_staff}, status=HTTPStatus.OK)
...
def user_grant_admin(request):
    ...
    if user.is_staff:
        ...
        user.is_staff = True
        user.save()
        log_user_permission_action(request.user, PermissionAction.GRANT, user.username)
        return JsonResponse(
            {
                "success_message": Status.SUCCESS.value
...
def user_revoke_admin(request):
    ...
    if (user.is_superuser) or (user.username == "admin"):
        ...
        user.is_staff = False
        user.save()
        log_user_permission_action(request.user, PermissionAction.REVOKE, user.username)
        return JsonResponse(
            {
                "success_message": Status.SUCCESS.value
                + "Admin permission revoked from user "
                + user.username
            },
            status=HTTPStatus.OK,
        )
...
def usernames(request):
    ...
    return JsonResponse({"usernames": usernames}, status=HTTPStatus.OK,)
...
def user_get_groups(request):
    ...
    return JsonResponse(permissions, status=HTTPStatus.OK,)
...
def all_user_groups(request):
    ...
    return JsonResponse({"groups": group_list}, status=HTTPStatus.OK,)
